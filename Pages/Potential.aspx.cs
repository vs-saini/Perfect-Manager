...
namespace Pages
{
    public partial class PagesPotentialMoney : Page
    {
        ...
        private void ShowNetWorth()
        {
            ...
            // Get MF investment
            const string investQuery = "SELECT SUM(Amount) FROM Expenses WHERE Category='Investment'"; // include MF
            ...
            // Get amount on which interest is payable
            // Get total earning from debtor in INR

            var elanceBalInr = elanceBal * _rate;
            var payPalBalInr = payPalBal * _rate;

            _netWorth = sbbjBankBal + sbiBankBal + sbbjCurrentBal + elanceBalInr + payPalBalInr + transferWise + debtAmount + ppf + pFd + pFd2 + investAmount + goldAmount + eRd + pRd;
            ...
            builder.Append(Utility.AmountToString(sbbjBankBal + sbbjCurrentBal + sbiBankBal));
            ...
            //lblNetWorthStatus.Text = finalNetText.Remove(finalNetText.LastIndexOf(" </li> ", StringComparison.OrdinalIgnoreCase));
            ...
            // Debtors with interest @ Rs. 2 per 100
        }
    }
}